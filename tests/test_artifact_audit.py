...
def test_audit_last_includes_search_read_and_refusal(tmp_path, monkeypatch, capsys):
    ...
    monkeypatch.setattr("sys.argv", ["syntaris", "--config", str(cfg), "audit-last"])
    assert cli.main() == 0
    payload = json.loads(capsys.readouterr().out)
    actions = [e["action"] for e in payload["audit"]]
    assert "artifact_find" in actions
    assert "artifact_read" in actions
